...
class DefaultDeeplinkParserTest {
    companion object {
        const val A_URI =
            "nexustalk://open/@alice:server.org"
        const val A_URI_WITH_ROOM =
            "nexustalk://open/@alice:server.org/!aRoomId:domain"
        const val A_URI_WITH_ROOM_WITH_THREAD =
            "nexustalk://open/@alice:server.org/!aRoomId:domain/\$aThreadId"
        const val A_URI_WITH_ROOM_WITH_THREAD_AND_EVENT =
            "nexustalk://open/@alice:server.org/!aRoomId:domain/\$aThreadId/\$anEventId"
        const val A_URI_WITH_ROOM_WITH_EVENT_AND_NO_THREAD =
            "nexustalk://open/@alice:server.org/!aRoomId:domain//\$anEventId"
        const val A_URI_WITH_ROOM_WITH_THREAD_AND_EVENT_AND_INVALID_CHARACTERS =
            "nexustalk://open/@a%2Flice:server.org/!a%2FRoomId:domain/\$a%2FThreadId/\$an%2FEventId"
    }
    ...
    fun `error cases`() {
        ...
        // Bad host
        assertThat(sut.getFromIntent(createIntent("nexustalk://close/@alice:server.org"))).isNull()
        // No session Id
        assertThat(sut.getFromIntent(createIntent("nexustalk://open"))).isNull()

        assertThrowsInDebug {
            // Invalid sessionId
            sut.getFromIntent(createIntent("nexustalk://open/alice:server.org"))
        }
        assertThrowsInDebug {
            // Empty sessionId
            sut.getFromIntent(createIntent("nexustalk://open//"))
        }
    }
